  ...
  <el-container class='home'>
    <el-header class='header'>
      ...
      <div class='right'>
        ...
        <el-button type="primary" size='mini' @click="loop"> <i class='iconfont iconicon_huanbi' /> 环比</el-button>
        <el-button type="primary" size='mini' @click="together"> <i class='iconfont iconicon_tongbi' /> 同比</el-button>
        <el-button type="primary" size='mini' @click="copy"> <i class='iconfont iconicon_fuzhi' /> 复制</el-button>
        <el-button type="primary" size='mini' @click="addColum"> <i class='iconfont iconicon_renwuxinzeng' /> 新增列
        </el-button>
        <el-button v-if="env !== 'production'" type="success" size='mini' @click="submitTitle('1')"> <i
            class='iconfont iconicon_baocun' /> 测试保存保存
        </el-button>
    ...
    <el-container>
      ...
      <el-main class='content'>
        <draggable-list ref='draggable' :numInfo='numInfo' group="a" :title="list" @getBack="drop"></draggable-list>
      </el-main>
    </el-container>
    <el-dialog title="添加菜单" :visible.sync="dialogVisible" width="30%">
      <el-form label-width="80px" style="font-size: 12px">
        <el-form-item label="名称">
          <el-input size="mini" :maxlength='20' v-model='titleInfo.titleName' :readonly='!!$route.query.name'
...
import { letMenu, addTitle, updateTitle, getTitleByName } from '@/api'
export default {
  name: 'home',
  data () {
    return {
  ...
  methods: {
    ...
    // 环比
    loop () {
      let data = this.$refs.draggable.titleBar
      let index = this.$refs.draggable.showIndex
      if (index !== null) {
        if (data[index].order === '3') {
          this.$message({
            type: 'warning',
            message: '该类型不支持环比'
          })
