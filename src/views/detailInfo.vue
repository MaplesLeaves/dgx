<template>
  <el-container class="detailInfo" :class="{Microservice:  env !== 'production'}">
          ...
          <div>
            ...
            <div>
              <div @click="showDiaolog">
                <el-input v-model="message.text" size="mini" :readonly="true"></el-input>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div>
              <el-date-picker v-model="date" type="monthrange" range-separator="至" value-format='yyyy-MM'
                start-placeholder="开始月份" size='mini' class='dateAll' end-placeholder="结束月份">
              </el-date-picker>
            </div>
          </div>
          <template v-for="(item, index) in serchBar">
            <div v-if="index < 3" :key="index">
              <!-- age -->
              <template v-if="item.isCondition === 'Y' && item.input_type === 'age'">
                <span class='titleName'>{{item.quota_name}}：</span>
                <div>
                  <el-input class="ageSerch" size="mini" style="width: 49%" v-model="message.startAge"></el-input>
                  <el-input class="ageSerch" size="mini" style="width: 49%" v-model="message.endAge"></el-input>
                </div>
              </template>
              <!--  comdate-->
              <template v-if="item.isCondition === 'Y' && item.input_type === 'comdate'">
                <span class='titleName'>{{item.quota_name}}：</span>
                <div>
                  <el-date-picker v-model="message[item.input_code]" type="monthrange" range-separator="至"
                    value-format='yyyy-MM' start-placeholder="开始月份" size='mini' style="width: 200px" class='dateAll'
                    end-placeholder="结束月份">
                  </el-date-picker>
                </div>
              </template>
              <!-- select -->
              <template v-if="item.isCondition === 'Y' && item.input_type === 'select'">
                <span class='titleName'>{{item.quota_name}}：</span>
                <div>
                  <el-select v-model="message[item.input_code]" multiple style="width: 240px" placeholder="请选择"
                    size="mini">
                    <el-option v-for="(val, j) in item.child" :key="val.val" :label="val.codename"
                      :value="val.codevalue">
                    </el-option>
                  </el-select>
                </div>
              ...
              <!-- joindate -->
              <template v-if="item.isCondition === 'Y' && item.input_type === 'joindate'">
                <span class='titleName'>{{item.quota_name}}：</span>
                <div>
                  <el-date-picker v-model="message.startJoinDate" style="width: 49%" size="mini" type="date"
                    value-format='yyyy-MM-dd' placeholder="选择日期">
                  </el-date-picker>
                  <el-date-picker v-model="message.endJoinDate" style="width: 49%" size="mini" type="date"
                    value-format='yyyy-MM-dd' placeholder="选择日期">
                  </el-date-picker>
                </div>
              </template>
              <!-- radio -->
              <template v-if="item.isCondition === 'Y' && item.input_type === 'radio'">
                <span class='titleName'>{{item.quota_name}}：</span>
                <div>
                  <el-select v-model=" message[item.input_code]" clearable placeholder="请选择" size="mini">
                    <el-option v-for="(val, j) in item.child" :key="val.val" :label="val.codename"
                      :value="val.codevalue">
                    </el-option>
                  </el-select>
              ...
              <!-- jobcode -->
              <template v-if="item.isCondition === 'Y' && item.input_type==='jobcode'">
                <span class='titleName'>{{item.quota_name}}：</span>
                <div>
                  <el-select v-model=" message[item.input_code]" multiple placeholder="请选择" size="mini">
                    <el-option v-for="(val, j) in item.child" :key="val.val" :label="val.codename"
                      :value="val.codevalue">
                    </el-option>
                  </el-select>
      ...
      <div class='tabAllInfo'>
        <el-table :header-cell-style="{background:'#F4F7FC', color: '#467EAF', padding:　'10px 0'}"
          ...
          <template v-for="(item, index) in tableBar">
            <el-table-column :label="item.cName || item.quota_name" :prop="item.column_code" v-if='!item.showEs'
              :key="index">
              <template slot-scope="scope" v-if='tableData.length'>
                {{
              scope.row[item.column_code] !== undefined || scope.row[item.input_code] !== undefined  ? scope.row[item.column_code] || scope.row[item.input_code] || 0 : ''
              }}
              </template>
            </el-table-column>
          </template>
...
import switchInfo from '@/components/switch.vue'

import { getTreeList, getData, findList, downLoad, seek, getTitleByName } from '@/api/detail.js'
...
export default {
  ...
  mounted () {
    ...
    if (this.env !== 'production') {
      findList().then(res => {
        ...
        let message = JSON.parse(JSON.stringify(this.nowTitle))
        var obj = {}
        let data = message.reduce(function (item, next) {
          obj[next.quota_name] ? '' : obj[next.quota_name] = true && item.push(next);
          return item;
        }, [])
        data.forEach(item => {
          if (item.isCondition === 'Y') {
            this.serchBar.push(item)
          }
        })
  ...
  methods: {
    ...
    getTabBar (val, index, item) {
      ...
      var obj = {}
      let data = message.reduce(function (item, next) {
        obj[next.quota_name] && obj.isCondition !== "Y" ? '' : obj[next.quota_name] = true && item.push(next);
        return item;
      }, [])
      data.forEach(item => {
        if (item.isCondition === 'Y') {
    ...
    load () {
      ...
      getData(this.message).then(res => {
        ...
        if (res.resultJsonHj.length) {
          this.tableData.unshift(res.resultJsonHj[0])
        }
    ...
    async serch (val) {
      ...
      let oldValue = this.listBar.concat(this.list, [])
      this.tableBar = oldValue;
      this.tableBar = this.tableBar.concat([{
        quota_name: '结算月',
        column_code: 'comdate'
      },
      {
      ...
      if (!this.message.comdateStart || !this.message.div || !this.message.date) {
        this.$message({
          message: '请选择完整的搜索条件',
          type: 'warning'
        })
      ...
      if (val) {
        ...
        let url = await downLoad(this.message).then(res => {
    ...
    loadExcel (res, fileName) {
      if (res) {
        ...
        let filename1 = nowDate + fileName + '.csv'
        if (this.myBrowser() === "IE" || this.myBrowser() === "Edge") {
          console.error(this.myBrowser())
